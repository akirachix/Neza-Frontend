...
const UpdateFileModal: React.FC<UpdateFileModalProps> = ({ fileData, onClose, onUpdate }) => {
  const initialUpdatedName = fileData && typeof fileData.name === 'string' ? fileData.name : '';

  const [updatedName, setUpdatedName] = useState<string>(initialUpdatedName);
  ...
  return (
    <>
      ...
      <div className="fixed top-0 left-0 w-full h-full flex items-center justify-center">
        <div className="bg-white p-5 shadow-md rounded-md border-gray-500  mr-10 w-1/3 h-1/2 flex flex-col justify-center items-center" style={{ boxShadow: '0px 4px 6px rgba(0, 0, 0, 0.1), 0px 8px 16px rgba(0, 0, 0, 0.2), 0px 12px 24px rgba(0, 0, 0, 0.3)' }}>
